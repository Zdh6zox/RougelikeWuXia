...
void FVoronoiDiagramSite::GenerateCentroid(FBox2D Bounds)
{
    ...
    for (auto Itr(Edges.CreateConstIterator()); Itr; ++Itr)
    {
        ...
        // (x, Min)
        if (FMath::IsNearlyEqual(LeftEndPoint.Y, Bounds.Min.Y, FLT_MIN) || FMath::IsNearlyEqual(RightEndPoint.Y, Bounds.Min.Y, FLT_MIN))
        {
            bHas_X_Min = true;
        }

        // (x, Max)
        if (FMath::IsNearlyEqual(LeftEndPoint.Y, Bounds.Max.Y, FLT_MIN) || FMath::IsNearlyEqual(RightEndPoint.Y, Bounds.Max.Y, FLT_MIN))
        {
            bHas_X_Max = true;
        }

        // (Min, y)
        if (FMath::IsNearlyEqual(LeftEndPoint.X, Bounds.Min.X, FLT_MIN) || FMath::IsNearlyEqual(RightEndPoint.X, Bounds.Min.X, FLT_MIN))
        {
            bHas_Min_Y = true;
        }

        // (Max, y)
        if (FMath::IsNearlyEqual(LeftEndPoint.X, Bounds.Max.X, FLT_MIN) || FMath::IsNearlyEqual(RightEndPoint.X, Bounds.Max.X, FLT_MIN))
        {
            bHas_Max_Y = true;
        }
    ...
    {
        SortedVertices.Add(FVector2D(Bounds.Min));
        bIsCorner = true;
    }
    ...
    {
        SortedVertices.Add(FVector2D(Bounds.Min.X, Bounds.Max.Y));
        bIsCorner = true;
    }
    ...
    {
        SortedVertices.Add(FVector2D(Bounds.Max.X, Bounds.Min.Y));
        bIsCorner = true;
    }
    ...
    {
        SortedVertices.Add(FVector2D(Bounds.Max));
        bIsCorner = true;
    }
    ...
    {
        CurrentVertex = FVector2D(SortedVertices[i]);
        NextVertex = FVector2D(SortedVertices[i + 1]);
